...
with open('/Users/javiermonge/Desktop/TransMoSYS/dictionary.yaml', 'r') as file:
...
with open('/Users/javiermonge/Desktop/TransMoSYS/config.yaml', 'r') as file: #Config File of OSeMOSYS Global
...
for country in dictionary.keys():
    ...
    #print("SpecifiedAnnualDemand for "+str(code)+" updated")
        ...
        # Generar SpecifiedDemandProfile a partir de YAML (no se leen datos previos)
    specified_demand = dictionary[country]['SpecifiedDemand']  # Ahora es una lista
    final_df_all = pd.DataFrame(columns=['REGION', 'FUEL', 'TIMESLICE', 'YEAR', 'VALUE'])
    
    for sector in specified_demand:
        # Generamos el código FUEL concatenando el sector con el country
        new_fuel_code = sector + country
        if sector[-3:] == 'ELC':
            new_fuel_code = sector + country + '02'
        print("Procesando:", new_fuel_code)
        
        # Generar filas para cada año y combination
        temp_rows = []
        for year in range(startYear, endYear + 1):
            for ts in combinations:
                ...
                print("Adding row:", len(combinations))
        df_sector = pd.DataFrame(temp_rows)
        final_df_all = pd.concat([final_df_all, df_sector], ignore_index=True)
    
    # Guardar el DataFrame final en el CSV (sobrescribe el archivo existente)
    final_df_all.drop_duplicates().to_csv('data/SpecifiedDemandProfile.csv', index=False, mode='w', header=True)
    ...
    for powerplant in PowerPlants:
        ...
        # Verificar que term tenga un valor. Si es None, asignamos una lista vacía u otro valor que necesites.
        if term is None:
            term = []
    ...
    for finaltech in FinalTechs:
        term = FinalTechs[finaltech]
        # Verificar que term tenga un valor. Si es None, asignamos una lista vacía u otro valor que necesites.
        if term is None:
            term = []
        filtered_df = df_country[df_country['TECHNOLOGY'].isin(term)]
        filtered_df.loc[:, 'REGION'] = 'GLOBAL'
        filtered_df.loc[:, 'TECHNOLOGY'] = "DEM" + finaltech + country
        if finaltech[-3:] == 'ELC':
            filtered_df.loc[:, 'TECHNOLOGY'] = "DEM" + finaltech + country + '02'
        filtered_df = filtered_df.groupby(['REGION', 'TECHNOLOGY'])['VALUE'].mean().reset_index()
        df_ctau = pd.concat([df_ctau, filtered_df]).drop_duplicates(subset=['REGION', 'TECHNOLOGY'], keep='last')
        df_ctau.to_csv('data/CapacityToActivityUnit.csv', index=False)
    #CapitalCost
